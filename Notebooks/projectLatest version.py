...
if hf_token:
    ...
    print("[WARN] HF_TOKEN not set; attempting to load dataset without explicit login.")
...
import torch
# from transformers import GPT2LMHeadModel, GPT2TokenizerFast  # unused (replaced by AutoModelForCausalLM)
import pandas as pd
import math

# Feature 81: LogRank using a lighter language model (distilgpt2)
...
def compute_logrank(text):
    """Compute approximate log-rank / loss score for a given text.

    We truncate to 128 tokens for efficiency. For empty or non-string inputs,
    we return 0.0.
    """
    ...
    with torch.no_grad():
        ...
        log_rank_score = outputs.loss.item()  # متوسط -log(prob) للتوكنات
...
values = []
max_rows = 1000   # نحسب لأول 1000 عينة فقط، والباقي نخليه 0 (لأسباب عملية في كولاب)
...
# عرض النتائج الأولى للتحقق
print(df[[feature_81_name]].head())
